// Simulation-only stand-in for the sync_fifo_w8_d32 IP referenced by
// rtl/input_dispatch.v (one per array row). Just wraps the existing
// generic rtl/sync_fifo.v at width=8/depth=32, renamed/re-pinned to match
// whatever vendor FIFO IP this name is meant to represent (clock/data/
// rdreq/wrreq/full/empty/q, no reset pin).
//
// Depth 32, not 16: input_buffer_32_bank delivers all 32 rows' words in
// the same cycle, and input_dispatch's skewed_start_cnt ramps each row's
// rdreq on one cycle apart - so row 31's FIFO has to hold up to 32 words
// (the whole preload burst) before its own drain even starts.
//
